...
-(void)scanForReference {
    ...
    for(Option *option in self.arrayOptions) {
        ...
        if(self.labelReference == nil) {
            self.labelReference = [option label];
            continue;
        }
        
        if([self max:[option label]] > [self max:self.labelReference])
            self.labelReference = [option label];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)aTableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // If there is reference, then it must be add into the UIView tree
    if(self.labelReference != nil) [cell.contentView addSubview:self.labelReference];
    
    [cell.contentView
            addConstraints:[Constraints centerCell:option withReference:self.labelReference]];
